...
def decrypt(mess):
	...
	des2 = ''.join(concatenate(
		[[chr(int(j, 16)) for j in wrap(i, 2) if int(j, 16) != 0] for i in temp_li]))

	return des2
def trippledesencrypt(mess):

    return encrypt(encrypt(encrypt(mess)))

def trippledesdecrypt(mess):
    return decrypt(decrypt(decrypt(mess)))
